...
async fn remove_last_video(ctx: &Context, channel: GuildChannel) -> bool {
    if let Ok(messages) = channel.messages(&ctx, |b| b).await {
        if let Some(found) = messages.iter().find(|message| message.content.contains("https://www.youtube.com/watch?v=")) {
    ...
    async fn respond_to_message(&self, ctx: &Context, message: &Message) -> serenity::Result<()> {
        ...
        if content.starts_with(YOUTUBE_PREFIX) {
            ...
            if content == "ta bort videon, bot." {
                if !remove_last_video(&ctx, channel).await {
...
impl EventHandler for NiceLittleBot {
    async fn message(&self, ctx: Context, message: Message) {
        if message.author.name == "trevlig liten bot" {
            return;
        }
